...
CandyShop.SlashCommands = (function(self, Candy, $) {
  ...
  self.defaultConferenceDomain = Candy.Core.getOptions().conferenceDomain;
  ...
  self.init = function(){
    ...
    $(Candy).on('candy:view.connection.status-' + Strophe.Status.CONNECTED, self.setDefaultConferenceDomain);
    $(Candy).on('candy:view.connection.status-' + Strophe.Status.ATTACHED, self.setDefaultConferenceDomain);
    ...
    $(Candy).bind('candy:view.message.before-send', function(e, args) {
      try {
          ...
          if (match !== null) {
            ...
            if ($.inArray(command, self.commands) !== -1) {
              ...
              sweetAlert({
                ...
                text: 'Sorry, "' + command + '" is not a valid known command.',
                timer: 2000
              });
            }
          }
        ...
        Candy.Core.error('[CandyShop SlashCommands] Exception caught in slash commands: ', er);
      }
    });
  };
  ...
  self.join = function(args) {
    ...
    if (typeof room !== 'undefined' && room !== '') {
      ...
      if (typeof password !== 'undefined' && password !== '') {
        ...
        if (typeof CandyShop.JoinOnResponse === "object") {
          CandyShop.JoinOnResponse.joinRoom({ roomJid: room }, false);
        } else {
